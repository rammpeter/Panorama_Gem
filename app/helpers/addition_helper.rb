...
module AdditionHelper
  ...
  def object_increase_object_name
    # TODO: PANORAMA_OBJECT_SIZES fest als Namen vorgeben
    schemas = sql_select_all "SELECT Owner, Table_Name FROM All_Tables WHERE Table_Name='PANORAMA_OBJECT_SIZES'"
    return "#{schemas[0].owner}.#{schemas[0].table_name}" if schemas.count > 0
