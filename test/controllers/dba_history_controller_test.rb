...
Rails.logger.info "####################### SQL-ID=#{sql_id} #{@hist_sql_id} #{@sga_sql_id_without_history} parsing_schema_name=#{parsing_schema_name}"
  ...
  test "list_sql_monitor_reports with xhr: true" do
    if get_db_version >= '12.1' && get_current_database[:management_pack_license] == :diagnostics_and_tuning_pack
      post '/dba_history/list_sql_monitor_reports', params: {format: :html, instance: 1, sql_id: @hist_sql_id, time_selection_start: @time_selection_start, time_selection_end: @time_selection_end, update_area: :hugo }
      assert_response :success
    end
  end
